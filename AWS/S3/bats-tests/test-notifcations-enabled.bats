...
EOF
  ...
  kubectl wait --timeout=5m --for=condition=Ready topic.sns.aws.crossplane.io/notification-bucket-topic
  run patch_and_apply_yaml "$(cat <<EOF
metadata:
  labels:
    test: $TESTNAME
EOF
  ...
  kubectl wait --timeout=5m --for=condition=Synced bucket.s3.aws.crossplane.io/notification-bucket
  assert_success
